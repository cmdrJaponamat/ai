...
## Уведомления HESK при действиях из PortalAL

- Нативное правило HESK проверено по `change_status.php`, `admin/change_status.php`, `admin/admin_reply_ticket.php`, `admin/admin_ticket.php`, `admin/assign_owner.php` и `admin/move_category.php`.
- Возобновление и прочие промежуточные смены статуса не рассылают писем ни заявителю, ни исполнителю: это нормальное поведение штатного HESK, а не потеря уведомления в PortalAL.
- Закрытие исполнителем уведомляет заявителя, если в HESK включен `notify_closed` (сейчас включен). Мост PortalAL приведён к этому правилу.
- Публичный ответ исполнителя уведомляет заявителя; ответ заявителя уведомляет назначенного исполнителя при его `notify_reply_my`. Эти сценарии уже использовали штатные функции HESK.
- Внутренняя заметка не приходит заявителю; она приходит другому назначенному исполнителю только при его `notify_note`. Мост PortalAL приведён к этому правилу.
- Передача заявки уведомляет нового исполнителя при `notify_assigned`, кроме передачи самому себе. Перемещение в категорию уведомляет автоматически назначенного исполнителя либо подходящих исполнителей нераспределённой категории — как в штатном HESK.

Фактическое изменение: коммит PortalAL `c402115`; на сервере HESK развёрнут `/opt/hesk-portal-bridge/index.php`, копия до изменения: `/opt/hesk-portal-bridge/index.php.bak-20260805-132351`. PHP lint и тесты PortalAL прошли. Перезапуск не требуется.

Откат только этой части:

```bash
sudo cp -p /opt/hesk-portal-bridge/index.php.bak-20260805-132351 /opt/hesk-portal-bridge/index.php
sudo php -l /opt/hesk-portal-bridge/index.php
