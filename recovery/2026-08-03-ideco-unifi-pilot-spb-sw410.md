...
Ожидаемый результат до подключения: VLAN 1460 имеет только tagged-порты `ether6,ether7`; VLAN 1461 идёт tagged `ether1` / untagged `ether11`; `ether6` и `ether11` inactive/no-link, PoE на них выключен. Ideco сохраняет доступность по `Leth1` / `192.168.203.82`.
...
Подключить строго по MAC-адресам сетевых карт Ideco (не по внутреннему имени ОС):

1. `1c:57:d8:18:de:00` ↔ `SPB-SW-410 ether6` — VLAN 1460 tagged.
2. `1c:57:d8:18:de:01` ↔ `SPB-SW-410 ether11` — VLAN 1461 untagged.

После каждого кабеля проверить линк, DHCP, маршрут до `172.31.146.1`, DNS и интернет одним тестовым клиентом. Только затем создать и назначить SSID на `SPB410IT`.
